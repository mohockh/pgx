...
END GAMEDEF""",
        ...
        for action in [3, 5, 8, 12]:  # Sample of no-limit actions
            ...
            if expected_bet_amount > 0 and expected_bet_amount <= state.stacks[current_player]:
                ...
                assert (
                    new_state.max_bet >= original_max_bet
                ), f"Max bet should increase or stay same for action {action}"
                ...
                assert (
                    new_state.bets[current_player] == expected_new_bet
...
END GAMEDEF""",
        ...
        for action in [0, 1, 2]:
            ...
            assert jnp.array_equal(
                state_from_betting_structure.bets, state_from_basic_action.bets
            ), f"Basic action {action} should produce identical bets"
            assert jnp.array_equal(
                state_from_betting_structure.stacks, state_from_basic_action.stacks
            ), f"Basic action {action} should produce identical stacks"
            assert jnp.array_equal(
                state_from_betting_structure.folded, state_from_basic_action.folded
        ...
        for action in [3, 6, 9]:
            ...
            if bet_amount > 0 and bet_amount <= state.stacks[current_player]:
                ...
                assert jnp.array_equal(
                    state_from_betting_structure.bets, state_from_nolimit_action.bets
                ), f"No-limit action {action} should produce identical bets"
                assert jnp.array_equal(
                    state_from_betting_structure.stacks, state_from_nolimit_action.stacks
...
END GAMEDEF""",
        ...
        for action in range(3, 13):
            bet_amount = env._calculate_nolimit_bet_amount(state, action)
            expected_affordable = (
                (bet_amount > 0) & (bet_amount <= state.stacks[current_player]) & ~state.folded[current_player]
            )
            assert (
                legal_actions[action] == expected_affordable
...
END GAMEDEF""",
        ...
        assert jnp.array_equal(
            vectorized_results, legal_actions[3:13]
...
END GAMEDEF""",
        ...
        assert jnp.array_equal(
            env.raise_multipliers[0], expected_preflop
...
END GAMEDEF""",
        ...
        for round_idx in [1, 2, 3]:
            assert jnp.array_equal(
                env.raise_multipliers[round_idx], expected_postflop
...
END GAMEDEF""",
        ...
        assert isinstance(
            env.betting_structure, universal_poker.NoLimitBettingStructure
