...
END GAMEDEF"""
        ...
        state = state.replace(stacks=state.stacks.at[0].set(0), all_in=state.all_in.at[0].set(True))
...
END GAMEDEF"""
        ...
        state = state.replace(stacks=state.stacks.at[0].set(0), current_player=0)
...
END GAMEDEF"""
        ...
        assert (
            state.folded[current_player] == True
        ), "Should be auto-folded at game start, not enough chips for big blind"
...
END GAMEDEF"""
    ...
    def test_legal_actions_edge_case_bet_equals_max_bet(self):
        ...
        expected_can_raise = total_chips > state.max_bet
        assert (
            legal_actions[universal_poker.RAISE] == expected_can_raise
        ), "Raise should depend on total chips vs max_bet"
    ...
    def test_legal_actions_edge_case_bet_exceeds_max_bet(self):
        ...
        state = state.replace(bets=state.bets.at[current_player].set(state.max_bet + 5))
...
END GAMEDEF"""
            ...
            for player_id in range(2):
                ...
                assert (
                    legal_actions[universal_poker.FOLD] == can_fold
                ), f"Fold logic failed for {description}, player {player_id}"
                assert (
                    legal_actions[universal_poker.CALL] == can_call
                ), f"Call logic failed for {description}, player {player_id}"
                assert (
                    legal_actions[universal_poker.RAISE] == can_raise
                ), f"Raise logic failed for {description}, player {player_id}"
...
END GAMEDEF"""
    ...
    def test_raise_action_zero_max_bet(self):
        ...
        assert (
            new_state.bets[current_player] == expected_raise_target
        ), f"Raise should be to {expected_raise_target} when max_bet is 0"
