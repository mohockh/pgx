...
END GAMEDEF"""
        ...
        # Correct logic: min raise should be previous max_bet + minimum_raise_increment
        # In this case, max_bet was 10, big blind is 10, so minimum raise = 10 + 10 = 20
        # But the raise amount is calculated as the delta, so player needs to put in 20 total
        assert new_state.max_bet == 20  # Current max bet after raise  
        assert new_state.bets[0] == 20   # Player 0 total bet
        assert new_state.stacks[0] == 80  # 95 - 15 (additional chips: 20 total - 5 already bet)
...
END GAMEDEF"""
        ...
        assert state.round == 2, "Game should have reached final round, 3 - 1 = 2"
...
END GAMEDEF"""
        ...
        assert state.round == 0, "Round should not have advanced after termination"
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("✓ Rewards array size test passed")
