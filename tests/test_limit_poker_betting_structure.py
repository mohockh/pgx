...
END GAMEDEF""")
        ...
        for action in [universal_poker.FOLD, universal_poker.CALL, universal_poker.RAISE]:
            ...
            assert jnp.array_equal(state_via_betting_structure.bets, state_via_env.bets), f"Bets should match for action {action}"
            assert jnp.array_equal(state_via_betting_structure.stacks, state_via_env.stacks), f"Stacks should match for action {action}"
            assert jnp.array_equal(state_via_betting_structure.folded, state_via_env.folded), f"Folded should match for action {action}"
...
END GAMEDEF""")
    ...
    def test_get_legal_actions_only_first_three_meaningful(self):
        """Test that only indices 0-2 are meaningful in legal actions array."""
        env = universal_poker.UniversalPoker(num_players=2, config_str="""GAMEDEF
limit
numplayers = 2
stack = 100 100
...
END GAMEDEF""")
        ...
        # Actions 3-12 should all be False for limit poker
        for i in range(3, 13):
            assert legal_actions[i] == False, f"Action {i} should be False in limit poker"
        
        # At least one of actions 0-2 should be True in a normal game
        basic_actions_available = jnp.any(legal_actions[:3])
        assert basic_actions_available, "At least one basic action should be available"
...
END GAMEDEF""")
        ...
        expected_can_call = (state.bets[current_player] <= state.max_bet) and (state.stacks[current_player] > 0) and ~state.folded[current_player]
...
END GAMEDEF""")
        ...
        assert jnp.array_equal(env.raise_multipliers, original_multipliers), \
...
END GAMEDEF""")
        ...
        assert isinstance(env.betting_structure, universal_poker.LimitPokerBettingStructure), \
...
END GAMEDEF"""
        ...
        for i, config in enumerate(configs):
            ...
            assert jnp.all(legal_actions[3:] == False), f"Config {i}: Actions 3+ should be False"
            assert isinstance(env.betting_structure, universal_poker.LimitPokerBettingStructure), \
