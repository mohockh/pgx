...
def create_poker_trainer(config: PokerConfig, env: pgx.core.Env, baseline_policy=None):
    ...
    def loss_fn(model_params, model_state, samples: Sample):
        (logits, value), model_state = forward.apply(
            model_params, model_state, samples.obs, is_training=True
        )
    ...
    def vectorized_evaluate_fn(batch_size: int):
        ...
        def _evaluate(model, rng_key: jnp.ndarray):
            ...
            def body_fn(val):
                ...
                jax.debug.print("==========================================================================")
                jax.debug.print("{batch_size}, {my_player}, {R}, {rewards}, {stacks}",
    ...
    return (forward, optimizer, vectorized_selfplay_fn, compute_loss_input_fn, 
            train_step, vectorized_evaluate_fn)


def parse_args():
    parser = argparse.ArgumentParser(description="Enhanced Universal Poker Training")
    parser.add_argument("--tensorboard_dir", type=str, default="runs/poker_training", 
                       help="Directory for TensorBoard logs")
    parser.add_argument("--max_num_iters", type=int, default=1000,
...
END GAMEDEF"""
    ...
    (forward, optimizer, vectorized_selfplay_fn, compute_loss_input_fn,
     train_step, vectorized_evaluate_fn) = create_poker_trainer(config, env, baseline_policy)
    ...
    for iteration in range(config.max_num_iters):
        ...
        for i in range(num_updates):
            ...
            minibatch = jax.tree_util.tree_map(
                lambda x: x[start_idx:end_idx], samples_shuffled
            )
        ...
        if iteration % config.log_interval == 0:
            print(f"Iter {iteration:4d}: Policy Loss={avg_policy_loss:.6f}, "
